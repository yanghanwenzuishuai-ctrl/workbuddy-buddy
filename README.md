...
Crates:
- **wb-buddy-core** — pure, content-free state derivation. 7 states
  (`idle/thinking/working/review/waiting/done/failed`); cross-session priority
  arbitration (`failed > waiting > working > review > thinking > done > idle`);
  TTL decay (Codex-pet lifetimes) so a state fades without an end-of-session
  event; decayed sessions are evicted.
...
Ships with a **library of 15 hand-designed cartoon buddies** (shiba, axolotl,
red panda, owl, penguin, and more). Switch anytime from the **menu-bar tray →
选择伙伴 / Choose buddy…** or by **double-clicking the pet**; your choice is
remembered. Design and add your own — see [docs/PET_SPEC.md](docs/PET_SPEC.md).
