...
The pet's look is a swappable **pet pack** (`pet.json` + a spritesheet). Drop
your own into `~/.workbuddy-buddy/pet/` and restart — no rebuild. There are 7
states (`idle · thinking · working · review · waiting · done · failed`). Full
authoring spec: **[docs/PET_SPEC.md](docs/PET_SPEC.md)**; the built-in reference
pet is generated by `assets/gen_sprite.py`.
